...
void DingoController::DingoControllerCallback(const geometry_msgs::Twist::ConstPtr& cmd)
{
	...
	scale_factor.data = 1.25; //was 2.5
	if(simulation == true)
	{
		scale_factor.data = 7.95; //scales so a twist of 1 outputs 1m/s in the simulation
	}
    ...
    velx.data = new_cmd.linear.x;
    vely.data = new_cmd.linear.y;
    velz.data = new_cmd.linear.z;
    omega.data = new_cmd.angular.z;  //command angular velocity
    omega_y.data = new_cmd.angular.y; //command for strafe left/right

    if(sqrt(velx.data*velx.data + vely.data*vely.data)<1) //limits wheel velocity to a value of 1 (when there was velx and vely the velocity could be sqrt(2))
    {
        vel.data = scale_factor.data*sqrt(velx.data*velx.data + vely.data*vely.data); //command velocity vector
    }
    else
    {
        vel.data = scale_factor.data*1;
    }
    ...
    else if(vel.data<.05 && abs(omega.data)>.05) //turn in place. Pressing left right triggers will turn wheels to position, then forward on left stick will give power to the wheels
    {
    vel_fl.data = -scale_factor.data*omega.data;
    vel_fr.data = scale_factor.data*omega.data;
    vel_bl.data = -scale_factor.data*omega.data;
    vel_br.data = scale_factor.data*omega.data;
    ...
    command_back_right_position.publish(pos_br);
        ...
        status.data = "Turning in place: no power to wheels";
        }

    }

    else if(vel.data<.05 && abs(omega_y.data)>.05) //strafe left/right. Pressing left/right on directional pad will turn wheels to position, then forward on left stick will give power to the wheels
    {
    pos.data = omega_y.data*M_PI/2; //turns wheel to +- pi/2 (note omega_y is either +-1)
    ...
    turn_in_place_command.data = scale_factor.data*velz.data;


        if(abs(velz.data) > .05) //if forward/back on left stick
        {
        ...
        status.data = "Strafe left/right: with power to wheels";
        }
        else
        {
        command_front_left_velocity.publish(zero);
        ...
        command_back_left_velocity.publish(zero);
        command_back_right_velocity.publish(zero);
        status.data = "Strafe left/right: no power to wheels";
        }
